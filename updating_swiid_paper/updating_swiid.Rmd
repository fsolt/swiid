...
single_obs_countries <- swiid_source %>% add_count(country) %>% filter(n == 1) %>% nrow() %>% setNames(c("one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten", "eleven", "twelve", "thirteen", "fourteen", "fifteen", "sixteen", " seventeen", "eighteen", "nineteen"), 1:19)[.]
```
...
```{r years_by_country, echo=FALSE, fig.cap="Countries by Years Covered in the SWIID Source Data \\label{countries_by_years}"}
swiid_source %>%
    mutate(country = if_else(stringr::str_detect(country, "United"),
                             stringr::str_replace(country, "((.).*) ((.).*)", "\\2.\\4."),
                             country)) %>% 
