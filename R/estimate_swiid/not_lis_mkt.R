...
rwecodes_x0 <- x0 %>% # region and wd_es combinations that exist in data for non-baseline countries
  select(rwecode, rwecode2) %>% 
  distinct()

rwecodes_all <- ineq2_m %>%
...
# Stan
start <- proc.time()
out1 <- stan(file = "R/estimate_swiid/not_lis_mkt.stan",
             data = source_data,
             seed = seed,
             iter = iter,
             warmup = warmup,
...
save(x, out1, file = str_c("data/not_lis_mkt_", iter/1000, "k_",
...
plot_tscs(x, out1, save_pdf = "paper/figures/ts_not_lis_mkt.pdf")
plot_tscs(x, out1)
