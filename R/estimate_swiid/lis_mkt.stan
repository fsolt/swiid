...
model {
  ...
  rho_s ~ lognormal(.25, .2);
  rho_kwe_hat ~ lognormal(.25, .2);

  for (k in 1:K) {
    if (kn[k] > 1) {
      gini[kt1[k]] ~ normal(.35, .125);                         // a random draw from N(.35, .125) in first year
      gini[(kt1[k]+1):(kt1[k]+kn[k]-1)] ~ normal(gini[(kt1[k]):(kt1[k]+kn[k]-2)], sigma_gini); // otherwise a random walk from previous year 
    } else {
      gini[kt1[k]] ~ normal(.35, .125);                         // a random draw from N(.35, .125)
    }
  }
