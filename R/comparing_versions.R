...
swiid_latest_82_71_70 <- swiid_summary_latest %>% 
  ...
  left_join(swiid_summary70 %>% 
            ...
            by = c("country", "year", "decade")) %>% 
  mutate(vlatest_minus82 = gini_latest - gini82,
         v82minus71 = gini82-gini71,
         v71minus70 = gini71-gini70,
         v82minus70 = gini82-gini70)

# Mean within country-year differences by decade, all countries
swiid_latest_82_71_70 %>% 
  group_by(decade) %>%
...
# Merge data, calculate within country-year differences across versions
swiid_latest_82_71_70_mkt <- swiid_summary_latest %>% 
  mutate(decade = floor(year/10) * 10) %>% 
...
swiid_latest_82_71_70_mkt %>%
  ...
  summarise(mean_diff = mean(vlatest_minus82, na.rm = TRUE),
  ...
  ggtitle("Latest Version Minus Version 8.2, By Country-Decade")
...
check_source <- function(cc) {
  ...
  anti_join(country_source_latest, country_source82)
}

check_source("Canada") %>% View()
