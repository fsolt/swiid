...
The new SWIID estimation routine described in the preceding section was written and revised to address these issues, but new observations are not added to the LIS quickly enough to allow for continuous testing of these revisions.
...
To grasp how _k_-fold cross-validation works, it helps to first understand the simpler form of cross-validation in which the available data are first divided into two groups of observations: the _training_ set and the _testing_ set.
...
```{r kfold_output, echo = FALSE, warning = FALSE, fig.height=3, fig.cap = "\\label{fig:kfold_output}_k_-fold Cross-Validation Results"}
...
```{r kfold_output_by_country, echo = FALSE, warning = FALSE, fig.height=3, fig.cap = "\\label{fig:kfold_output_by_country}_k_-fold Cross-Validation Results, Observations Excluded by Country"}
