...
Country-years with more observations in the source data have more information to update the estimate $\hat{G}_{bkt}$ based on $\hat{\rho}_{s}$, $\hat{\rho}_{kwe}$, the product of $\hat{\rho}_{kw}$ and $\hat{\rho}_{re}$, and/or $\hat{\rho}_{rwe}$.
If no observation is available the random walk process spans the gap between last year to appear in the source data and the next year to appear.
...
This measurement error was taken into account by modelling the true quantity of each observation in the source data as normally distributed around the observed Gini with deviation equal to the associated standard error.
