...
Coverage is relatively good in all of the years from 2000 to 2016, with at least 80 countries observed per year, before dropping to `r swiid_source %>% filter(year==2017) %>% count(country) %>% nrow()` countries for 2017 and just `r swiid_source %>% filter(year==2018) %>% count(country) %>% nrow()` for 2018.^[
