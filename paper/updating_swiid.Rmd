...
However, in light of the SWIID's goal of providing information on income inequality for the broadest possible sample of countries and years, the ubiquity of the Gini makes it the only plausible choice for this purpose.
...
The starting point for the SWIID's estimates are two collections of Gini indices from the LIS, one of market income inequality and one of disposable income inequality, each using the square-root equivalence scale [@LIS2018].
The LIS has meticulously harmonized its survey microdata to be as comparable cross-nationally as possible; inequality observations calculated from these harmonized microdata therefore serve as the baseline for the SWIID.^[
...
These two LIS series anchor the SWIID's estimates of market and disposable income inequality. 
...
These relationships fall into four categories:

  1. The ratio $\rho_{s}$ for each series $s$---again defining a series as a group of observations from the same country calculated using the same methodology---that include country-years that also appear in the LIS baseline,^[
Because the two LIS collections do not contain the identical country-years (the collection of disposable income inequality Ginis has `r swiid_source %>% filter(str_detect(series, "LIS") & equiv_scale == "sqrt" & welfare_def == "disp") %>% nrow()` observations, while that for market income inequality has only `r swiid_source %>% filter(str_detect(series, "LIS") & equiv_scale == "sqrt" & welfare_def == "market") %>% nrow()`), these series vary somewhat when estimates for each of the two welfare definitions are being generated.] 
such that
  \[
  G_{bkt} \sim \mathcal{N}(\rho_{s} \times G_{skt},\,\sigma_s^{2})
  \]
  where $G_{bkt}$ is the LIS Gini for baseline welfare definition $b$ in country $k$ at time $t$, $G_{skt}$ is the Gini from series $s$ in that same country $k$ and time $t$, and $\sigma_s^{2}$ is a series-specific error term.  That is, $\rho_{s}$ can be understood as the average ratio of the LIS baseline to the series $s$ in all years that the two overlap.  This is the most specific adjustment: the relationship is not only country-specific and includes the welfare definition and equivalence scale, but because each series uses a consistent methodology, such factors as the comprehensiveness of the income definition and the reporting period are also taken into account.
  
  2. The ratio $\rho_{kwe}$, estimated as the average ratio of the LIS baseline to Ginis with the same country-year for country $k$ with welfare definition $w$ and equivalence scale $e$.  Of course, like $\rho_{s}$, the first category of relationships, $\rho_{kwe}$ can only be estimated in the 50 countries that are included in the LIS.  In version 8.0, $\rho_{kwe}$ can be estimated for all observations in the source data for countries included in the LIS, and $\rho_{s}$ can be estimated for nearly half of these observations.

  3. The ratio $rho_{kw}$, estimated as the average ratio of Ginis in country $k$ with welfare definition $w$ to Ginis with the same country-year and equivalence scale but the baseline welfare definition $b$, and the ratio $rho_{rwe}$, the average ratio of the LIS baseline


<!-- The differences in the treatment of items such as non-monetary income and imputed rents, though much smaller than those resulting from the factors used to generate the SWIID's categories, nevertheless can be consequential.  For example, consider the 174 country-years of Gini indices found in Version 3b of the \citet{UNU2014} dataset that overlap with the coverage of the LIS and were calculated on the basis of household-adult-equivalent disposable income, the same combination of equivalence scale and welfare definition employed by the LIS.  Over a fifth of these were substantively and statistically significantly different from the LIS values [see @Solt2016, 1278].  Treating these data as if comparable by placing them in a single category fails to take into account the error that their differences introduce. -->

<!-- A potential partial solution, at least for more data-rich countries, would be to generate within-country estimates on the basis of the unique series of data provided by individual sources rather than collecting these series into the categories currently used.  To do this, the research team will first need to identify the breaks in those series provided by each source when that source's underlying methodology changes, that is, what \citet[389-393]{Atkinson2009} call ``breaks in continuity.''  For example, \citet{Eurostat2014a} indicates breaks in the time series of its data for France occurred in 2001, 2004, and 2008.  Rather than a single consistent series, then, these three breaks mean the Eurostat data for France actually consist of four separate series (1995-2000, 2001-2003, 2004-2007, and 2008-2013) that are not comparable with each other.\footnote{That these breaks result in year-to-year jumps ranging in size from an entire Gini index point (in 2001) to more than three points (in 2008) is another indication of the importance of harmonization.}  The source data will be recoded accordingly.  Next, the imputation model's code for generating within-country estimates through loess and linear regression will be expanded so that $\rho_{abit}$ are predicted not simply for all available combinations of categories $a$ and $b$ but for all available combinations of \emph{series}.  As these analyses are already performed in parallel, the dramatically larger number of combinations should not prove an obstacle. -->



<!-- Making use of all of this available information is not straightforward.  One cannot simply ignore its many sources of incomparability. -->

<!-- For each country included in the LIS, the SWIID's estimates are based exclusively on relationships observed within the country.  First, for each country's series that share observed years with the LIS, the relationship $rho_as$ between the LIS data and the series were estimated as  -->
