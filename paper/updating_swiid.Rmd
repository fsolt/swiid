...
abstract: "_Objective_: This article documents wide-ranging revisions to the Standardized World Income Inequality Database (SWIID), which seeks to maximize the comparability of income inequality estimates for the broadest possible coverage of countries and years. _Methods_: Two _k_-fold cross-validations, by observation and by country, are used to evaluate the SWIID's success in predicting the Luxembourg Income Study (LIS), recognized in the field as setting the standard for comparability. _Results_: The cross-validations indicate that for both the relatively data-rich countries included in the LIS and the relatively data-poor countries outside the LIS, the SWIID's estimates are both "close to the target" and "have good credible intervals." _Conclusion_: Given its superior coverage and comparability, the SWIID remains the optimum source of data for broadly cross-national research on income inequality."
...
But even when observations are drawn from a single source does not necessarily make them comparable: as @Atkinson2009 [, 392] observed, inequality statistics can suffer from "a break in continuity" if "the underlying source is the same, but the methods changed."
...
The functions of the \texttt{purrr} package [@Henry2019, included within the widely-used \texttt{tidyverse} package] makes incorporating the uncertainty in the SWIID's estimates into an analysis conducted in \textsf{R} completely straightforward, scarcely more complex than omitting it.
Step-by-step instructions on how to use these tools, complete with examples, are included in the data download.

Despite its clear advantages for broadly cross-national research, the SWIID will not be the best choice for all work on income inequality.
Those examining trends in income inequality within only a single country, in particular, will often find that national sources exist that meet their needs.
To help this latter group of researchers identify these sources, a second web application focused on the SWIID source data is now available at the author's website.
In addition to displaying graphically all of the available income inequality data for each country, the SWIID source data web application identifies the welfare definition and equivalence scale that each source employs and provides direct links to the electronic versions of these sources. 
